...
def get_euro_rate_bonbast():
    ...
    if not match:
        raise ValueError("نرخ یورو پیدا نشد")

    return int(match.group(1))

@st.cache_data(ttl=300)
def cached_euro_rate():
    return get_euro_rate_bonbast()

st.title("BeautyAI")
st.subheader("محاسبه‌گر قیمت تمام‌شده و قیمت پیشنهادی فروش")

try:
    eur_rate = cached_euro_rate()
    st.success(f"نرخ یورو از Bonbast: {eur_rate:,.0f} تومان")
except Exception:
    st.warning("نرخ زنده Bonbast دریافت نشد. نرخ را دستی وارد کن.")
